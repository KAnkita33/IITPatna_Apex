...
def search(request):
    ...
    ob2=product_women.objects.all()
    if request.method == "POST":
        hashtag = request.POST.get('hashtag')
        ob = product.objects.filter(hashtag__contains=hashtag)
        ob2 = product_women.objects.filter(hashtag__contains=hashtag)
    else:
        ob = product.objects.all()
        ob2 = product_women.objects.all()
    
    return render(request, "search.html", {'ob1': ob, 'ob2': ob2})
